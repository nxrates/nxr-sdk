...
use nxr_sdk::synth::{Leg, LegTick, SynthPath, compute_synth_tick as rs_compute};
...
pub fn compute_synth_tick(
    ...
    for (k, v) in snapshots.iter() {
        ...
        let snap = if let Ok(tup) = v.downcast::<PyTuple>() {
            ...
            let mid: f64 = if tup.len() > 2 {
                tup.get_item(2)?.extract()?
            } else {
                (bid + ask) / 2.0
            };
            let conf: u16 = if tup.len() > 3 {
            ...
            let bid: f64 = d
                .get_item("bid")?
                .ok_or_else(|| PyValueError::new_err("snapshot missing bid"))?
                .extract()?;
            let ask: f64 = d
                .get_item("ask")?
                .ok_or_else(|| PyValueError::new_err("snapshot missing ask"))?
                .extract()?;
            let mid: f64 = match d.get_item("mid")? {
                Some(m) => m.extract()?,
                None => (bid + ask) / 2.0,
            };
            let conf: u16 = match d.get_item("conf")? {
                Some(c) => c.extract()?,
                None => 10_000,
            };
            LegTick {
    ...
    let Some(out) = rs_compute(&path, &map) else {
        return Ok(None);
    };
