...
pub trait VolSource {
    ...
    /// Bin index for the given unix-ms timestamp. Clamps to
...
impl<'a, S: VolSource + ?Sized> MtfParkinsonCalculator<'a, S> {
    ...
    #[inline]
    pub fn find_index_for_ts(&self, timestamp_ms: i64) -> usize {
        self.source.find_index_for_ts(timestamp_ms)
    }
...
mod tests {
    ...
    impl VolSource for StaticSource {
        ...
        fn find_index_for_ts(&self, _ts: i64) -> usize { self.0.len().saturating_sub(1) }
    }
