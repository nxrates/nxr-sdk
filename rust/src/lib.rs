//! # NXR SDK
//!
//! Consumer SDK for NX Rates live market data feeds.
//!
//! ## Transports
//!
...
//!     }
//!
//!     // UDP multicast: raw MITCH frames on LAN
...
// Re-export MITCH types consumers need.
            ...
            match msg {
                Ok(Message::Binary(data)) => {
                    match Self::decode_frame(&data) {
                        ...
                        None => continue, // skip malformed frames
                    }
                }
                Ok(Message::Ping(_)) => continue, // tungstenite auto-responds with pong
                Ok(Message::Close(_)) => return None,
                Ok(_) => continue, // ignore text/pong
                Err(e) => {
...
impl<T: Pod + Send + 'static> MulticastStream<T> {
    ...
    pub async fn recv(&self) -> Option<T> {
        ...
        // Over-allocate to detect oversized datagrams.
